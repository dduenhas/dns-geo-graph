...
| 3 | **Caminho absoluto do usuário publicado** (`C:\Users\<usuário>\…`) no README gerado e numa nota do vault — vazamento de informação local num repositório público | 🟡 médio | passou a ser relativo (`<pasta-do-clone>/grafo`) em `04_build_vault.py` e `05_write_readme.py` |
| 4 | **Coordenadas 3D não determinísticas** (`hash()` de `str` é salgado por processo) — quebrava a reprodutibilidade prometida | 🟡 médio | CRC32 estável em `03_build_graph.py`; verificado: as 1.222 posições de nós sem geo são idênticas entre execuções |
